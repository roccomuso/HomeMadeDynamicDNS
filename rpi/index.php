...
$file = file_get_contents('./ip.txt', FILE_USE_INCLUDE_PATH);
$dati = explode(" ", $file);
...
case "log_richieste.txt":
if (file_exists('./log_richieste.txt'))
	echo nl2br(file_get_contents('./log_richieste.txt')); // la funzione nl2br mette un <br/> prima di ogni \n
else echo "Il file non esiste!";
die();
break;

default:
$url = 'http://'.$dati[0]."/".$u; //$_GET['url'];
//controllo che l'host sia raggiungibile
$host = $dati[0];
$port = 80;
$waitTimeoutInSeconds = 4;
if ($fp = fsockopen($host,$port,$errCode,$errStr,$waitTimeoutInSeconds)) {
  // It worked
} else {
  // It didn't work
  die("<h2>Host non raggiungibile o configurazione NAT assente...</h2>");
}
fclose($fp);
break;
}
...
file_put_contents("log_richieste.txt", $url." - ".$_SERVER['REMOTE_ADDR']." - ".date("d-m-Y H:i:s")."\n", FILE_APPEND);
